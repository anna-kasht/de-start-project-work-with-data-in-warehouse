/*создадим временную таблицу из двух новых источников данных*/
DROP TABLE IF EXISTS tmp_sources_new;
CREATE TEMP TABLE tmp_sources_new AS 
...
JOIN external_source.customers AS c ON cpo.customer_id=c.customer_id;
/*добавим данные по заказчикам из временной таблицы в уже существующую таблицу,*/
/*если в существующей таблице уже есть такой заказчик, то обновим данные по нему*/
MERGE INTO dwh.d_customer AS d
USING (SELECT DISTINCT customer_name, customer_address, customer_birthday, customer_email 
	   FROM tmp_sources_new) n
ON d.customer_name = n.customer_name AND d.customer_email = n.customer_email 
WHEN MATCHED THEN
  UPDATE SET customer_address = n.customer_address, 
			 customer_birthday = n.customer_birthday, load_dttm = current_timestamp
WHEN NOT MATCHED THEN
  INSERT (customer_name, customer_address, customer_birthday, customer_email, load_dttm)
  VALUES (n.customer_name, n.customer_address, n.customer_birthday, n.customer_email, current_timestamp);
-- добавим данные по мастерам из временной таблицы в уже существующую таблицу,
-- если в существующей таблице уже есть такой мастер, то обновим данные по нему
MERGE INTO dwh.d_craftsman AS d
USING (SELECT DISTINCT craftsman_name, craftsman_address, craftsman_birthday, craftsman_email 
	   FROM tmp_sources_new) n
ON d.craftsman_name = n.craftsman_name AND d.craftsman_email = n.craftsman_email 
WHEN MATCHED THEN
  UPDATE SET craftsman_address = n.craftsman_address, 
			 craftsman_birthday = n.craftsman_birthday, load_dttm = current_timestamp
WHEN NOT MATCHED THEN
  INSERT (craftsman_name, craftsman_address, craftsman_birthday, craftsman_email, load_dttm)
  VALUES (n.craftsman_name, n.craftsman_address, n.craftsman_birthday, n.craftsman_email, current_timestamp);
-- добавим данные по продуктам из временной таблицы в уже существующую таблицу,
-- если в существующей таблице уже есть такой продукт, то обновим данные по нему
MERGE INTO dwh.d_product AS d
USING (SELECT DISTINCT product_name, product_description, product_type, product_price 
	   FROM tmp_sources_new) n
ON d.product_name = n.product_name AND d.product_description = n.product_description AND d.product_price = n.product_price 
WHEN MATCHED THEN
  UPDATE SET product_type = n.product_type, 
			 load_dttm = current_timestamp
WHEN NOT MATCHED THEN
  INSERT (product_name, product_description, product_type, product_price, load_dttm)
  VALUES (n.product_name, n.product_description, n.product_type, n.product_price, current_timestamp);
/* создадим временную таблицу с фактами */
DROP TABLE IF EXISTS tmp_sources_fact_new;
CREATE TEMP TABLE tmp_sources_fact_new AS 
...
-- добавим данные по фактам из временной таблицы в уже существующую таблицу,
-- если в существующей таблице уже есть такой факт, то обновим данные по нему
MERGE INTO dwh.f_order f
USING tmp_sources_fact_new t
...
-- создадим дополнительную таблицу, куда будет записываться время последнего обновления данных в витрине на основе даты их загрузки в DWH
DROP TABLE IF EXISTS dwh.load_dates_customer_report_datamart;
CREATE TABLE IF NOT EXISTS dwh.load_dates_customer_report_datamart (
id BIGINT GENERATED ALWAYS AS IDENTITY NOT NULL,
load_dttm DATE NOT NULL,
CONSTRAINT load_dates_customer_report_datamart_pk PRIMARY KEY(id));
-- напишем запрос на создание витрины данных
...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    id_top_craftsman BIGINT NOT NULL, -- идентификатор самого популярного мастера ручной работы у заказчика
...
WITH
dwh_delta_cust AS ( -- определяем, какие данные были изменены в витрине или добавлены в DWH. Формируем дельту изменений
    SELECT     
...
dwh_update_delta_cust AS ( -- делаем выборку заказчиков, по которым были изменения в DWH. По этим заказчикам данные в витрине нужно будет обновить
    SELECT     
            dd.exist_customer_id AS customer_id
            FROM dwh_delta_cust dd 
                WHERE dd.exist_customer_id IS NOT NULL        
),
dwh_delta_insert_result_cust AS ( -- делаем расчёт витрины по новым данным. Этой информации по заказчикам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT  
            ...
            T4.craftsman_id AS id_top_craftsman,
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконные функции для определения самой популярной категории товаров и индентификатора популярного мастера
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самую популярную категория товаров у заказчика за месяц. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров 
                                            dd.customer_id AS customer_id_for_product_type, 
                                            dd.product_type, 
                                            COUNT(dd.product_id) AS count_product
                                            FROM dwh_delta_cust AS dd
                                ...
                                INNER JOIN (
                                	SELECT -- Эта выборка поможет определить id самого популярного мастера ручной работы у заказчика. Для данных этой выборки можно применить оконную функцию
                ...
                ) AS T4 WHERE T4.rank_count_product = 1 AND T4.rank_count_craftsman = 1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров и id попупярного мастера
),
dwh_delta_update_result_cust AS ( -- делаем перерасчёт для существующих записей витрины, так как данные обновились за отчётные периоды. Логика похожа на insert, но нужно достать конкретные данные из DWH
    SELECT 
            T4.customer_id AS customer_id,
            T4.customer_name AS customer_name,
            T4.customer_address AS customer_address,
            ...
            T4.report_period AS report_period 
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров и индентификатора поуплярного мастера
                        *,
                        RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_product DESC) AS rank_count_product,
                        ROW_NUMBER() OVER(PARTITION BY T2.customer_id ORDER BY count_craftsman DESC) AS rank_count_craftsman
                        FROM (
                                ...
                                FROM (
                                    SELECT     -- в этой выборке достаём из DWH обновлённые или новые данные по заказчикам, которые уже есть в витрине
                                            ...
                                            FROM dwh.f_order fo 
                                                ...
                                                INNER JOIN dwh.d_customer dcs ON fo.customer_id = dcs.customer_id 
                                                INNER JOIN dwh.d_product dp ON fo.product_id = dp.product_id
                                                INNER JOIN dwh_update_delta_cust ud ON fo.customer_id = ud.customer_id
                                ) AS T1
                                    GROUP BY T1.customer_id, T1.customer_name, T1.customer_address, T1.customer_birthday, T1.customer_email, T1.report_period
                            ) AS T2 
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у заказчика. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров
                                            dd.customer_id AS customer_id_for_product_type, 
                                            dd.product_type, 
                                            COUNT(dd.product_id) AS count_product
                                            FROM dwh_delta_cust AS dd
...
insert_delta AS ( -- выполняем insert новых расчитанных данных для витрины 
    INSERT INTO dwh.customer_report_datamart (
        ...
        report_period
            ...
            count_order_not_done,
            report_period 
            FROM dwh_delta_insert_result_cust
),
update_delta AS ( -- выполняем обновление показателей в отчёте по уже существующим мастерам
    UPDATE dwh.customer_report_datamart SET
        ...
        id_top_craftsman = updates.id_top_craftsman,
    ...
    FROM (
        SELECT 
            ...
            count_order_not_done,
            report_period 
            FROM dwh_delta_update_result_cust) AS updates
